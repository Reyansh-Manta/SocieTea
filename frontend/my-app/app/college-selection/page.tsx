...
export default function CollegeSelection() {
    ...
    const fetchColleges = async (currentPage: number, search: string, shouldAppend: boolean) => {
        setLoading(true)
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? `http://${window.location.hostname}:9000` : 'http://localhost:9000')}/api/v1/college/get-colleges?search=${encodeURIComponent(search)}&page=${currentPage}&limit=${PAGE_SIZE}`)
    ...
    const handleSubmitFormat = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? `http://${window.location.hostname}:9000` : 'http://localhost:9000')}/api/v1/college/submit-email-format`, {
                method: "POST",
                headers: {
