...
export default function OrgPage() {
                ...
                if (user) {
                    ...
                    if (user.Organization) {
                        ...
                        if (foundOrg) {
                            ...
                            const eventsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? `http://${window.location.hostname}:9000` : 'http://localhost:9000')}/api/v1/events/get-org-events`, {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
    ...
    const handleFollow = async () => {
        if (!org || !user) return;
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? `http://${window.location.hostname}:9000` : 'http://localhost:9000')}/api/v1/orgs/toggle-membership`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
